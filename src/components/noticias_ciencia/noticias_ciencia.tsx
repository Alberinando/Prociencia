...
const Ciencia: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/noticias.json')
      ...
      .then(data => {
        const todasNoticias = data.noticiasCiencia;
        ...
        const randomIndexCategoria1 = Math.floor(Math.random() * todasNoticias.length);
        setNoticiaCategoria1(todasNoticias[randomIndexCategoria1]);

        const randomIndexCategoria2 = Math.floor(Math.random() * todasNoticias.length);
        setNoticiaCategoria2(todasNoticias[randomIndexCategoria2]);

        const randomIndexCategoria3 = Math.floor(Math.random() * todasNoticias.length);
        setNoticiaCategoria3(todasNoticias[randomIndexCategoria3]);

        const randomIndexCategoria4 = Math.floor(Math.random() * todasNoticias.length);
        setNoticiaCategoria4(todasNoticias[randomIndexCategoria4]);
      })
      ...
      {noticiaPrincipal ? (
        <div className='ml-[20px] mr-[20px]'>
          <div className='grid grid-cols-12 gap-4 mt-[150px] mb-[20px]'>
            <h1 className='text-roxo-p08 grid col-start-2 col-span-3 text-[20px] font-black text-4xl'>Ciência</h1>
          </div>
          ...
          <div className='grid grid-cols-12 gap-4 mt-[20px]'>
            <a href="/MaisNoticias" rel="noopener noreferrer" className="text-roxo-p08 grid col-start-2 col-span-8 font-black ">
              <div className=' flex flex-row'>
                ...
                <img src="/assets/Imagens_aleatorias/seta-direita.png" alt="imagem de seta" className='ml-2'/>
              </div>
            </a>
          </div>
        </div>
