...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/noticias.json')
      ...
      .then(data => {
        const noticiasPolitica = data.noticiasPolitica;

        const randomIndexPrincipal = Math.floor(Math.random() * noticiasPolitica.length);
        setNoticiaPrincipal(noticiasPolitica[randomIndexPrincipal]);

        const randomIndexCategoria1 = Math.floor(Math.random() * noticiasPolitica.length);
        setNoticiaCategoria1(noticiasPolitica[randomIndexCategoria1]);

        const randomIndexCategoria2 = Math.floor(Math.random() * noticiasPolitica.length);
        setNoticiaCategoria2(noticiasPolitica[randomIndexCategoria2]);

        const randomIndexCategoria3 = Math.floor(Math.random() * noticiasPolitica.length);
        setNoticiaCategoria3(noticiasPolitica[randomIndexCategoria3]);

        const randomIndexCategoria4 = Math.floor(Math.random() * noticiasPolitica.length);
        setNoticiaCategoria4(noticiasPolitica[randomIndexCategoria4]);
      })
